...
from arch.dcgan_mini import Generator
from arch.dcgan_mini import Discriminator
...
class train_config(ut_cfg.config):
    def __init__(self):
        ...
        self.path_save_mdid = "dcganMNIST" + "%02d%02d"%(localtime.tm_mon, localtime.tm_mday)
    ...
    def noise_generate_func(self, *size):
        ...
        # U(-1 ,1)
        return torch.rand(*size).to(self.device) #* 2 - 1
    ...
    def create_dataset(self, istrain):
        ...
        self.transforms = transforms.Compose([transforms.Resize((self.height_in, self.width_in)),
                               transforms.ToTensor(),
                               # transforms.Normalize((0.5,), (0.5,))
        ]) # deplicate(0.0, 1.0) -> (-1, 1)
        q_dataset = bld.toy_dataset(bld.ToyDatasetName.MNIST, 
            pRoot = self.dtroot, pIstrain = istrain, 
            pTransform= self.transforms, pDownload= True
        )
        return q_dataset
...
if __name__ == "__main__":
    ...
    # prepare nets
    gm_netG = Generator(gm_cfg.latent_num)
    gm_netD = Discriminator()
    ...
    try:
        ...
        for epoch_i in range(gm_cfg.training_epoch_amount):
            ...
            gm_cfg.log_in_board( "traing loss", 
                {"dcGAN_d_loss": avgD_loss, 
                "dcGAN_g_loss": avgG_loss, 
                },  epoch_i
            )
